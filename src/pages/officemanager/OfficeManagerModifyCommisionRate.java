...
public class OfficeManagerModifyCommisionRate extends VBox {
    ...
    public OfficeManagerModifyCommisionRate(Stage stage) {
        // query db here to update the rate figures
        currentFixedCommissionRate = DBMethods.getFixedRate();
        currentVariableCommissionRate = DBMethods.getVariableRate();
        ...
        fixedCommissionRateButton.setOnAction(event -> {
            ...
            try {
                ...
                int count = DBMethods.getCommissionRowCount("fixed");
                if (count > 0) {
                    // update the fixed commission rate row
                    DBMethods.updateCommissionRate("fixed", Integer.parseInt(fixedCommissionRate));
                } else {
                    // create a new row for the fixed commission rate
                    DBMethods.createCommissionRate("fixed", Integer.parseInt(fixedCommissionRate));
                }
        ...
        variableCommissionRateButton.setOnAction(event -> {
            ...
            try {
                ...
                int count = DBMethods.getCommissionRowCount("variable");
                if (count > 0) {
                    // update the variable commission rate row
                    DBMethods.updateCommissionRate("variable", Integer.parseInt(variableCommissionRate));
                } else {
                    // create a new row for the variable commission rate
                    DBMethods.createCommissionRate("variable", Integer.parseInt(variableCommissionRate));
                }
            } catch (SQLException e) {
                e.printStackTrace();
            }
